...
async function findOrOpenSavedLinksTab() {
...
async function sendCurrentWindowTabsToSavedLinks() {
    ...
    const allTabs = await chrome.tabs.query({});
    const existing = allTabs.find((t) => typeof t.url === 'string' && t.url.startsWith(SAVED_LINKS_URL));
    if (existing && existing.id !== undefined) {
...
chrome.commands?.onCommand.addListener((command) => {
    switch (command) {
        ...
        case 'send-tabs-to-saved-links':
            sendCurrentWindowTabsToSavedLinks().catch((err) => console.error('send-tabs-to-saved-links failed:', err));
            break;
        default:
